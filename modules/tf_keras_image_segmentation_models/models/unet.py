"""
####################################################################################
U-Net Image Segmentation Model with Tensorflow and Keras.
####################################################################################
Research Paper/Documentation: https://arxiv.org/abs/1505.04597
...
################################################################################
def unet_encoder(encoder_type='Default',
                 input_tensor=None,
    ...
    if not (isinstance(encoder_filters, tuple) or isinstance(encoder_filters, list)):
        ...
        raise ValueError('The items of the `encoder_filters` argument cannot be less '
                         'than the `num_blocks` argument.')
...
def unet_decoder(num_classes,
    ...
    """
    Instantiates decoder architecture for the U-Net segmmantation model.
    Args:
    ...
    if decoder_type.lower() == 'transpose':
        for decoder_block_id in range(num_blocks):
            skip_conn = skip_connection_layers[decoder_block_id]
...
def unet(num_classes,
         ...
         output_activation=None):
    """
    Instantiates the U-Net architecture for semantic segmantation tasks.
    ...
    Args:
        ...
        encoder_filters: (optional) a list containing filter sizes for each  
                         encoder and decoder blocks. Default to 
                         [32, 64, 128, 256, 512].
        decoder_type: (optional) one of 'transpose' (to use Conv2DTanspose 
        ...
        output_activation: (optional) activation for output layer.
                            ...
                            the value of the 'num_classes' argument.
    Returns:
        model: keras unet segmentation model
    ...
    # Validate and preprocess arguments
    # -------------------------------------------------------------------------#
    # 1. num_classes - check unet_decoder functon
    # 2. encoder_type - check unet_encoder functon
    # 3. input_shape 
    ...
    # 5. num_blocks - check unet_encoder functon
    # 6. encoder_weights - check unet_encoder functon
    # 7. encoder_freeze - check unet_encoder functon
    # 8. encoder_filters - check unet_encoder functon
    # 9. decoder_type - check unet_decoder functon
    # 10. num_decoder_block_conv_layers - check unet_decoder functon
    # 11. decoder_activation - check unet_decoder functon
    # 12. decoder_use_skip_connection - check unet_decoder functon
    # 13. decoder_use_batchnorm - check unet_decoder functon
    # 14. decoder_dropout_rate - check unet_decoder functon
    # 15. output_activation - check unet_decoder functon
    ...
    encoder_model, encoder_model_output_layer, skip_connection_layers = unet_encoder(
    ...
    outputs = unet_decoder(
        num_classes=num_classes,
        decoder_input = encoder_model_output_layer,
